...
if undetermined
    tensor_G = xcross_spectra(y_nyquist, y_nyquist, num_fft);
    for mm = 1:Mantenna 
        tensor_G_fib = squeeze(abs(tensor_G(mm,mm,:)));
        noise_power_est = min(tensor_G_fib);
        tensor_G(mm,mm,:) = tensor_G(mm,mm,:) - noise_power_est;  
    end
    options_ft = 1;
