...
R = 3;               % Number of transmissions
Bw = 1.6e8;
  
fc = [-5e8, 1e8, 6e8, -3e8, 4e8, 7e8]; % carrier frequency 
...
for rr = 1:R
    ...
    for ff = 1:num_fft
        ...
        if brute_force
            ...
            [spectra_est,~] = Naive_omp(Vandermonde_C, alias_pxx_observation_r(:,ff), k_sparse);
...
xlabel('Frequency (Hz)', 'FontName', 'Times New Roman', 'FontSize', 14);
ylabel('Normalized PSDs', 'FontName', 'Times New Roman', 'FontSize', 14);
title(['The Proposed, $R=3$, $M=4$, NMSE = ', sprintf('%0.4g', nmse)], 'Interpreter', 'latex', ...
      'FontName', 'Times New Roman', ...
      'FontSize', 14);
set(gca, 'FontName', 'Times New Roman', 'FontSize', 14);
legend off;
